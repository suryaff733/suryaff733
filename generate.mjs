...
const USERNAME = process.env.GH_USERNAME || "suryaff733";
...
async function fetchWeeks() {
  if (TOKEN) {
    try {
      ...
      console.warn("GraphQL fetch failed, falling back to public profile...", err.message);
    }
  }

  // Fallback: fetch public contribution calendar without auth
  const res = await fetch(`https://github.com/users/${USERNAME}/contributions`);
  if (!res.ok) {
    throw new Error(`Failed to fetch public contributions for ${USERNAME}: ${res.status} ${res.statusText}`);
  }
  ...
  if (weeks.length === 0) {
    throw new Error(`Could not parse contribution weeks for ${USERNAME}`);
  }
  return weeks;
}
